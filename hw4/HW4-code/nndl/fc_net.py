...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dims=100, num_classes=10,
    ...
    # ================================================================ #
    self.params['W1'] = np.random.normal(scale=weight_scale, size = (input_dim, hidden_dims), loc=0.0)
    self.params['W2'] = np.random.normal(scale=weight_scale, size = (hidden_dims, num_classes), loc=0.0)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(1, self.num_layers + 1): 
        ...
        #add batchnorm
        if self.use_batchnorm and i != self.num_layers: #if i = self.num_layers, index is out of range?  
            gam = "gamma" + str(i) 
            beta = "beta" + str(i) 
            self.params[gam] = np.ones(hidden_dims[i-1]) 
            self.params[beta] = np.zeros(hidden_dims[i-1]) #0, 1 wouldn't work lol 
        ...
        #update params 
        self.params[W] = np.random.normal(loc=0.0, scale=weight_scale, size=size)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers + 1): 
        ...
        if i == self.num_layers: 
            ...
            if self.use_batchnorm : 
                appendVar = affine_batchnorm_forward(hidden_param, self.params[W], self.params[b], self.params[gam], self.params[beta], self.bn_params[i-1])
            else: 
                appendVar = affine_relu_forward(hidden_param, self.params[W], self.params[b])
        ...
        if i != self.num_layers and self.use_dropout > 0:
            ...
            dropout_cache.append(dropout_forward(hidden[i-1], self.dropout_param)[1])
            
            #below code is cleaner, but has more error 
            #this is because we need to first modify hidden[i-1] (line 336) before passing it into dropout_forward (line 337) 
            #I didn't do that below, and hence my error is larger 
            
#             dropoutAppendVar = dropout_forward(hidden[i-1], self.dropout_param)
...
#             dropout_cache.append(dropoutAppendVar[1])
    ...
    for i in range(self.num_layers, 0, -1): 
        ...
        if i == self.num_layers: 
            dh, grads[W], grads[b] = affine_backward(dscores, hidden_cache[self.num_layers-1])
        else: 
            
            #droppout happens before batchnorm / baseline NN backprop 
            
            if self.use_dropout > 0: 
                dh = dropout_backward(dh, dropout_cache[i-1])
            
            if self.use_batchnorm: 
                dh, grads[W], grads[b], grads[gam], grads[beta] = affine_batchnorm_backward(dh, hidden_cache[i-1])
            
            else: 
                dh, grads[W], grads[b] = affine_relu_backward(dh, hidden_cache[i-1])
        
        dhidden.append(dh) 
        
        grads[W] += self.reg * self.params[W] #need to have tuple unpacking above for this to work

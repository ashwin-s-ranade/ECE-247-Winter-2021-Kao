  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers + 1): 
        ...
        if i == 1: 
            ...
            scores = affine_forward(hidden[i-2], self.params[W], self.params[b])[0]
        else: 
            hidden_param = hidden[i-2]
        
        if i != self.num_layers: 
            hidden.append(affine_relu_forward(hidden_param, self.params[W], self.params[b])[0])
            
        #then do hidden_cache 
        if i == self.num_layers: 
            obj = affine_forward(hidden[i-2], self.params[W], self.params[b])[1]
        else: 
            if i == 1: 
                par = X 
            else: 
            ...
            obj = affine_relu_forward(par, self.params[W], self.params[b])[1]
